...
def header(filename,indexename):   
        ...
        for author in item.findall("author"):
            for book in author.findall("book_title"):
                for sentence in book.findall("sentence"):
                    balise="text id="+item.attrib.get('id')+"/author id="+author.attrib.get('id')+"/book_title id="+book.attrib.get('id')+"/sentence id="+sentence.attrib.get('id')
    ...
    conn.close()
################################################################################################################
header("corpus_العصر الحديث.xml",'indexe_العصر الحديث.db')#CHANGE
