...
def header(filename,indexename):   
        ...
        for author in item.findall("news-title"):
            for book in author.findall("category"):
                for sentence in book.findall("sentence"):
                    balise="news id="+item.attrib.get('id')+"/news-title id="+author.attrib.get('id')+"/category id="+book.attrib.get('id')+"/sentence id="+sentence.attrib.get('id')
    ...
    conn.close()
################################################################################################################
header("corpus_news_العصر الحديث.xml",'indexe_news_العصر الحديث.db')#CHANGE
